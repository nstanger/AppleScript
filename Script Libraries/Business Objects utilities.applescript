...
on getTeachingPeriod(validPeriods)
	...
	if periodCode = "N" then
		-- Non-standard period codes seem a bit weird Ñ TTRPT02 fails to return anything
		-- if you ask for period code ÒNSÓ. ÒALLÓ works, however. Simple solution: return the
		-- index for ÒAllÓ instead of ÒNÓ (but still return the ÒNÓ, as thatÕs used to open the
