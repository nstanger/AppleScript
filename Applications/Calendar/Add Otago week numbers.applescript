...
repeat until theYear ³ 1905 and theYear ² 2039
	set theYear to display dialog "This script will add week numbers to the iCal 'Week numbers' calendar for the year below." default answer year of (current date) with icon 1 with title "Add week numbers for this year"
...
set thePeriods to {Â
	...
	{id:"S2", name:"Semester 2", period:{start:28, finish:41, break:{start:35, finish:35}}} Â
		}
...
repeat with i from 0 to 51
	...
	tell application "Calendar"
		...
		tell calendar "Week numbers"
			make new event with properties {summary:"Week " & weekNumber & " - Day " & dayNumber, start date:nextSunday, end date:nextSunday, allday event:true}
			
			if (semesterString ­ "") then
				make new event with properties {summary:semesterString, start date:nextSunday, end date:nextSunday, allday event:true}
			end if
		end tell
